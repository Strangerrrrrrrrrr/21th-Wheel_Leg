...
{  
    ...
    {
        pwm_init(MOTOR_LEFT_SENSORLESS_A_PHASE_H, FPWM, 0);                                         // 上桥统一初始化为 PWM 输出 频率默认20k 0占空比
    ...
    {
        pwm_init(MOTOR_RIGHT_SENSORLESS_A_PHASE_H, FPWM, 0);                                        // 上桥统一初始化为 PWM 输出 频率默认20k 0占空比
...
{      
    ...
    pwm_set_duty(MOTOR_LEFT_SENSORLESS_A_PHASE_H, (uint16)(phase_a > 0 ? phase_a : 0));         // 配置A相上桥占空比    
      
    pwm_set_duty(MOTOR_LEFT_SENSORLESS_B_PHASE_H, (uint16)(phase_b > 0 ? phase_b : 0));         // 配置B相上桥占空比 
    
    pwm_set_duty(MOTOR_LEFT_SENSORLESS_C_PHASE_H, (uint16)(phase_c > 0 ? phase_c : 0));         // 配置C相上桥占空比 
...
{      
    ...
    pwm_set_duty(MOTOR_RIGHT_SENSORLESS_A_PHASE_H, (uint16)(phase_a > 0 ? phase_a : 0));        // 配置A相上桥占空比    
      
    pwm_set_duty(MOTOR_RIGHT_SENSORLESS_B_PHASE_H, (uint16)(phase_b > 0 ? phase_b : 0));        // 配置B相上桥占空比 
    
    pwm_set_duty(MOTOR_RIGHT_SENSORLESS_C_PHASE_H, (uint16)(phase_c > 0 ? phase_c : 0));        // 配置C相上桥占空比 
...
{       
    ...
    {        
        ...
        {            
            float wait_time = 30.0f - (motor_left.sensorless_duty - 0.4f) * 25.0f / 0.6f;                                                       // 计算 延时换相时间
...
void sensorless_motor_right_trig_isr(void)
{
    ...
    {        
        ...
        {            
            float wait_time = 30.0f - (motor_right.sensorless_duty - 0.4f) * 25.0f / 0.6f;                                                      // 计算 延时换相时间
